...
class Keyboard:    
    def __init__(self, key_assignment=key_assignment, comfort_score=combined_score, total_distance_traveled=total_distance_traveled, combined_score=combined_score, key_assignment_letters =key_assignment_letters, key_assignment_non_letters = key_assignment_non_letters, key_assignment_statics = key_assignment_statics,
    key_coordinates=key_coordinates, key_comfort=key_comfort):
        ...
        self.key_assignment_letters = key_assignment_letters
        self.key_assignment_non_letters = key_assignment_non_letters
        self.key_assignment_statics = key_assignment_statics
        self.key_coordinates = key_coordinates
        self.key_comfort = key_comfort
    ...
    def calc_shift_distance(self, last_key, char):
        ...
        total_distance = 0
        if 'SHIFT' in self.key_assignment:
            if last_key:  # If there's a previous key, calculate the distance from it to SHIFT and then to the current character
                distance_to_shift = self.calc_distance(last_key, 'SHIFT')
                distance_to_char = self.calc_distance('SHIFT', current_key)
                total_distance = distance_to_shift + distance_to_char
            else:  # If it's the first character, only calculate the distance from SHIFT to the character
                total_distance = self.calc_distance('SHIFT', current_key)
